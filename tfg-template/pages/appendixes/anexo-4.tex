\section{Ejemplo de funcionamiento de LaTeX}

% Citar una referencia
Esto es una referencia bibliográfica \cite{bibex}. 

Podemos definir una nota a pie de página.\footnote{Esta es una nota a pie de página de ejemplo.}
...
\begin{figure}
  ...
  \begin{minipage}{0.45\textwidth}
   \centering
     ...
     \includegraphics[clip=true,width=\textwidth]{img/triangulo_grande_bb.pdf}\\
  ...
  \begin{minipage}{0.45\textwidth}
   \centering
     \includegraphics[clip=true,width=\textwidth]{img/triangulos_separados_bb.pdf}\\
  ...
  \end{minipage}
    ...
    \includegraphics[clip=true,width=0.5\textwidth]{img/triangulos_unidos_bb.pdf}\\
    \footnotesize (c)
...
\begin{lstlisting}[ caption={Endpoint REST para recuperar un libro por su id}, label={lst:rest}]
@GetMapping("/{id}")
...
\end{lstlisting}
El código~\ref{lst:rest} es un ejemplo en Java.
...
Esto es un ejemplo de texto \tutor{Esto es un comentario del tutor} \alumno{Esto es un comentario del alumno}